...
var TNFile = {
	...
	_make: function _make(name, type, data) {
		this.name = name;
		this.type = type;
		this.data = data;
...
var DBItemFile = {
	...
	_make: function _make(name, type, data, path, mimeType) {
		this.name = name;
		this.type = type;
		this.data = data;
		this.path = path;
...
//export
var saveFileTo = function saveFileTo(args) {
...
var openFile = function openFile(args, callback) {
	var system = args.system || this.DEFAULTSYSTEM;
...
var removeFile = function removeFile(args, callback) {
	var system = args.system || this.DEFAULTSYSTEM;
	var storage = db.transaction(system, "readwrite").objectStore(system);
...
var getStaticFilePointerFromPath = function getStaticFilePointerFromPath(args, callback) {
	var system = args.system || this.DEFAULTSYSTEM;
	var storage = db.transaction(system, "readonly").objectStore(system);
	var request = storage.get(args.path);
...
var getStaticFilePointerFromFile = function getStaticFilePointerFromFile() {
	// @ToDo: implement this!! basically just call URL.createObjectURL...
};

var getFileList = function getFileList(args, callback) {
	var system = args.system || this.DEFAULTSYSTEM;
	var storage = db.transaction(system, "readonly").objectStore(system);
	var list = [];
...
var getFileListSince = function getFileListSince(args, callback) {
	var path = args.path;
...
var fileSystem = exports.fileSystem = {
