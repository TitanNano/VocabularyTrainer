...
var HashEvent = {
	...
	_make: function _make(type, path) {
		this.type = type;
		this.path = path;
	},

	trigger: function trigger(count) {
		var path = this.path;
...
//exports
var Interface = {
	mount: function mount(path, enter, exit, persistent) {
		if (path instanceof Array) path.forEach(function (item) {
			hash.actions.push({ path: item, enter: enter, exit: exit, persistent: persistent, active: false });
		});else hash.actions.push({ path: path, enter: enter, exit: exit, persistent: persistent, active: false });
		return true;
	},
	...
	restore: function restore() {
		...
		if (hash && hashString != hash.target.location.hash) hash.target.location.hash = hashString;else this.trigger();
	},
	...
	bind: function bind(object) {
		...
		object.addEventListener('hashchange', function () {
			var hashPath = null;
			if (this.location.hash === "") hashPath = '#!/'.split('/');else hashPath = this.location.hash.split('/');
			...
			for (var i = 0; i < hash.path.length; i++) {
				...
				if (difference) events_lost.push(new HashEvent(HashEvent_LOST, path));else if (hash.path[i] == hashPath[i]) continue;else if (hash.path[i] != hashPath[i]) {
					difference = true;
			...
			for (i = 0; i < hashPath.length; i++) {
				...
				if (difference) events_add.push(new HashEvent(HashEvent_ADD, path));else if (hashPath[i] == hash.path[i]) continue;else if (hashPath[i] != hash.path[i]) {
					difference = true;
...
var hash = exports.hash = Interface;
