...
exports.default = {
    zipper: {
        unzip: function unzip(file, callback) {
            var fileReader = null;
            if ($$.FileReader) {
        ...
        createBlob: function createBlob(zipRecord, onProgress) {
        ...
        readFileHeader: function readFileHeader(reader, zipRecord) {
        ...
        writeFileHader: function writeFileHader(fileHeader, writer) {
            writer.centralDirectorySize += writer.writeBytes([0x02014b50, 4], [fileHeader.version_made, 2], [fileHeader.version_needed, 2], [fileHeader.flag, 2], [fileHeader.compression, 2], [fileHeader.time, 2], [fileHeader.date, 2], [fileHeader.crc, 4], [fileHeader.compressed_size, 4], [fileHeader.uncompressed_size, 4], [fileHeader.name_length, 2], [fileHeader.extra_length, 2], [fileHeader.comment_length, 2], [fileHeader.diskNumbeStart, 2], [fileHeader.internal_attributes, 2], [fileHeader.external_attributes, 4], [fileHeader.relative_offset, 4]);
        ...
        readFile: function readFile(reader, zipRecord) {
        ...
        writeFile: function writeFile(file, writer) {
            writer.writeBytes([0x04034b50, 4], [file.version_a, 2], [file.flag, 2], [file.compression, 2], [file.time, 2], [file.date, 2], [file.crc, 4], [file.compressed_size, 4], [file.uncompressed_size, 4], [file.name_length, 2], [file.extra_length, 2]);
            writer.writeBuffer(this.encodeFromString(file.name), file.extra, file.data);
        },

        readExtraData: function readExtraData(reader, zipRecord) {
            var extra_length;
        ...
        writeExtraData: function writeExtraData(extraData, writer) {
            writer.writeBytes([0x08064b50, 4], [extraData.extra_length, 4]);
            writer.writeBuffer(extraData.extra);
        },

        readDigitalSignature: function readDigitalSignature(reader, zipRecord) {
            var size;
        ...
        writeDigitalSignature: function writeDigitalSignature(digitalSignature, writer) {
            writer.writeBytes([0x05054b50, 4], [digitalSignature.size, 2]);
            writer.writeBuffer(digitalSignature.data);
        },

        readZip64EndOfCentralDirectory: function readZip64EndOfCentralDirectory(reader, zipRecord) {
        ...
        writeZip64EndOfCentralDirectory: function writeZip64EndOfCentralDirectory(ecd, writer) {
            writer.writeBytes([0x06064b50, 4], [ecd.record_size, 8], [ecd.version_made, 2], [ecd.version_needed, 2], [ecd.disk, 4], [ecd.dir_start_disk, 4], [ecd.dir_entry_count, 8], [ecd.size_central_dir, 8], [ecd.offset, 8]);
        },

        readEndOfCentralDirectory: function readEndOfCentralDirectory(reader, zipRecord) {
            var comment_length;
        ...
        writeEndOfCentralDirectory: function writeEndOfCentralDirectory(ecd, writer) {
            writer.writeBytes([0x06054b50, 4], [ecd.disk, 2], [ecd.start_disk, 2], [ecd.entries, 2], [ecd.total_entries, 2], [writer.centralDirectorySize, 4], [ecd.start_offset, 4], [ecd.comment_length, 4]);
            writer.writeBuffer(this.encodeFromString(ecd.comment));
        },

        readSignature: function readSignature(reader) {
            return reader.readBytes(4);
        },

        readNextBySignature: function readNextBySignature(signature, reader, zipRecord) {
        ...
        ZipReader: function ZipReader(buffer) {
            this.buffer = buffer;
        ...
        intToHexString: function intToHexString(int) {
            return (int + 0x10000).toString(16).substr(-4).toUpperCase();
        },

        decodeAsString: function decodeAsString(buffer) {
            return String.fromCharCode.apply(null, new $$.Uint8Array(buffer));
        },

        encodeFromString: function encodeFromString(string) {
